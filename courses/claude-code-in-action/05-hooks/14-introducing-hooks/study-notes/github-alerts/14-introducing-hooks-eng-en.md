...
```mermaid
graph LR
    A[Your Prompt] --> B[Claude Model picks tool]
    B --> C{PreToolUse Hook}
    C -->|Allow| D[Tool Execution]
    C -->|Block| E[Error sent back to Claude]
    D --> F[PostToolUse Hook]
    F --> G[Result back to Claude]
```
...
```mermaid
graph LR
    A[Claude wants to read file] --> B{PreToolUse Hook}
    B -->|✅ Allow| C[Read proceeds]
    B -->|❌ Block| D[Error sent back to Claude]
```
...
```mermaid
graph LR
    A[Claude just wrote file] --> B[PostToolUse Hook]
    B --> C[Run prettier]
    B --> D[Run linter]
    C --> E[Feed results back to Claude]
    D --> E
    E --> F[Claude auto-corrects]
```
