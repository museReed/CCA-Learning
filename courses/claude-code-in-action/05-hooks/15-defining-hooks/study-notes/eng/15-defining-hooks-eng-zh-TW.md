...
1. **選對 hook 類型** — 「prevent」/「must not」→ PreToolUse；「after」/「transform」/「log」→ PostToolUse
2. **選對 matcher** — 知道哪些內建工具暴露哪些能力
3. **理解 stdin 資料流** — command 收到的是 JSON，不是 command-line arguments
4. **正確應用 exit codes** — 0 = 允許，2 = 封鎖（僅 PreToolUse）
...
**C** — 需要 PreToolUse 才能在執行前封鎖。`Read` 和 `Grep` 都能存取檔案內容，所以 matcher 必須涵蓋兩者。檢查 `.env` 是在 command 邏輯裡做的，不是在 matcher。
...
**B** — 資料標準化必須在工具回傳資料後進行（PostToolUse）。Hook 處理原始輸出並在 Claude 使用前標準化日期格式。
