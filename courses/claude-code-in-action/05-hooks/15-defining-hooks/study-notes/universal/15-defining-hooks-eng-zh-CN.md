...
![四步驟流程](../../visuals/four-step-process-zh-TW.svg)
...
![Exit Code 語意](../../visuals/exit-code-semantics-zh-TW.svg)
*圖：Exit code 語意 — 0 代表放行���工具正常執行），2 代表攔截（stderr 回饋給 Claude）。*
...
- 这创造了反馈回路：封锁 + 解释 → Claude 尝试不同方法

> ⚠️ **Exit code 2 仅适用于 PreToolUse**
>
...
这个配置的意思是："在 Claude 调用 Read 或 Grep 之前，执行 `read_hook.js`。脚本从 stdin 读取 tool call JSON，决定允许或封锁，然后以 exit code 0 或 2 结束。"
...
在 CCA 考试中，hook 定义题通常测试你是否能：

1. **选对 hook 类型** — "prevent"/"must not" → PreToolUse；"after"/"transform"/"log" → PostToolUse
2. **选对 matcher** — 知道哪些内建工具暴露哪些能力
3. **理解 stdin 数据流** — command 收到的是 JSON，不是 command-line arguments
4. **正确应用 exit codes** — 0 = 允许，2 = 封锁（仅 PreToolUse）

核心考试哲学：
- **Architecture > Prompt** — hook 是结构性强制执行，不是建议
- **Deterministic > Probabilistic** — exit code 2 是保证封锁，不是"请不要"
...
**C** — 需要 PreToolUse 才能在执行前封锁。`Read` 和 `Grep` 都能访问文件内容，所以 matcher 必须涵盖两者。检查 `.env` 是在 command 逻辑里做的，不是在 matcher。
...
**B** — 数据标准化必须在工具返回数据后进行（PostToolUse）。Hook 处理原始输出并在 Claude 使用前标准化日期格式。
