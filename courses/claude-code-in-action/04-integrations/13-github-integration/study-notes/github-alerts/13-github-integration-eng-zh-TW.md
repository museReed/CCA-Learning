...
```mermaid
graph LR
    A[/install-github-app] --> B[PR 帶 2 個 workflow 檔]
    B --> C[Merge]
...
這是 Task Statement 1.1（agentic loops）在 CI/CD 環境的應用。
...
- D 用了 blanket permission，在 GitHub Actions 裡不可用
...
**B** — 四個元件都需要：環境設定、MCP 設定、明確權限、custom instructions。
...
**B** — 在 GitHub Actions 裡，在 `mcp_config` 設定 MCP server 不夠。該 server 的每個工具都必須在 `allowed_tools` 逐一列出。

- A 不正確——Playwright 可以在 GitHub Actions 裡使用
- C 不正確——Claude 每次 action 運行都是全新啟動
- D 可能有幫助但不是 root cause
