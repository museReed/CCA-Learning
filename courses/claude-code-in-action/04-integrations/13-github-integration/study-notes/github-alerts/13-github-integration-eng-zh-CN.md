...
```mermaid
graph LR
    A[/install-github-app] --> B[PR 带 2 个 workflow 文件]
    B --> C[Merge]
    C --> D["@claude Mention Action"]
    C --> E[PR Review Action]
```
...
| GitHub Actions (CI) | 逐一列出工具 | `allowed_tools: "mcp__playwright__browser_click,..."` |
...
这是 Task Statement 1.1（agentic loops）在 CI/CD 环境的应用。
...
- D 用了 blanket permission，在 GitHub Actions 里不可用
...
**B** — 四个组件都需要：环境设置、MCP 设置、明确权限、custom instructions。
...
**B** — 在 GitHub Actions 里，在 `mcp_config` 设置 MCP server 不够。该 server 的每个工具都必须在 `allowed_tools` 逐一列出。

- A 不正确——Playwright 可以在 GitHub Actions 里使用
- C 不正确——Claude 每次 action 运行都是全新启动
- D 可能有帮助但不是 root cause
