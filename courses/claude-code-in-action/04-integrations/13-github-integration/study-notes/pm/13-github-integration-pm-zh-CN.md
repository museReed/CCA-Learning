...
1. **流程自动化决策** — 知道 Claude 能在 code review 和 issue triage 的哪些环节取代人工
2. **品质关卡设计** — 自动化 PR review 建立了结构性的品质层
3. **权限治理** — CI/CD 需要明确的工具权限（没有 blanket access）
4. **团队 workflow 影响** — `@claude` mention 改变了开发者跟 issue 和 PR 交互的方式
...
任何人都可以在 issue 或 PR 留言中提及 `@claude` 来触发 Claude。Claude 会：
- 分析请求
- 建立 step-by-step 计划（在留言中可见）
- 执行计划（跑代码、测试 app、读文件）
- 在 issue/PR 里回报结果
...
每个 PR 自动触发 Claude review 变更。Claude 会：
- 分析 diff
- 检查潜在问题
- 贴出结构化的 review

**PM 应用场景**：品质关卡 — 不管团队忙不忙，每个 PR 都有基本 review。在时间压力下人类可能遗漏的结构性问题，自动化 review 都能抓到。
...
> 💡 **PM Takeaway**
>
> `allowed_tools` 设置是权限边界。在 CI/CD 里，每个工具都必须逐一列出——没有「允许全部」的选项。这是刻意的安全设计。如果团队加了新的 MCP 工具，`allowed_tools` 清单也必须更新。
...
2. **CI 里的端到端测试** — 讲师设置 Claude 启动 app、通过 Playwright 打开浏览器、测试 UI 功能——全在 GitHub Action 里完成。这是一个完整的测试回路。
3. **明确权限是不可妥协的** — 讲师强调在 GitHub Actions 里，每个 MCP 工具都必须逐一列出。没有捷径。这是 CI/CD 最重要的设置细节。
...
**B** — 在 GitHub Actions（用 `-p` flag 的非交互模式）里，每个 MCP 工具都必须在 `allowed_tools` 逐一列出。本地 `.claude/settings.local.json` 的 blanket `mcp__playwright` 权限不适用。
...
**A** — `@claude` mention workflow 正好就是为这个场景设计的。QA 可以贴截图、提及 `@claude`，Claude 就会调查——导航到 app、测试功能、直接在 issue 里回报结果。

- B 是预防性的，但没有解决现有的 bug triage
- C 是文档化，不是自动化
- D 是开发期的控制，不是 QA workflow

**PM 重点**：`@claude` mention 把 bug triage 从「需要开发者介入的阻塞活动」变成「AI 自动化的 workflow」。QA 更快得到响应，开发者被打断的次数更少。
</details>
