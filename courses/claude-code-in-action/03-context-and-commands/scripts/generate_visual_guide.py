#!/usr/bin/env python3
"""
Generate visual study guides from video + SRT for Chapter 03-context-and-commands.
...
Processes 2 videos in 03-context-and-commands:
  - 10-controlling-context (66 entries)
  - 11-custom-commands (26 entries)
"""
...
VIDEO_CONFIG = [
    (
        "10-controlling-context",
        "Controlling_context_pIw5RYqx.srt",
        "Controlling_context_pIw5RYqx.mp4",
        CONTROLLING_CONTEXT_ZH,
        66,
        "Controlling Context — 控制對話流向",
    ),
    (
        "11-custom-commands",
        "Custom_commands_ChNLn8eT.srt",
        "Custom_commands_ChNLn8eT.mp4",
        CUSTOM_COMMANDS_ZH,
        26,
...
def generate_markdown(entries: list[dict], unit_dir: str, title: str, output_path: Path):
    ...
    lines = [
        ...
        "| 項目 | 內容 |",
        "|------|------|",
        f"| 課程 | claude-code-in-action / 03-context-and-commands / {unit_dir} |",
        f"| 影片 | {title.split(' — ')[0]} |",
        f"| 字幕數 | {len(entries)} 段 |",
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate visual guides for 03-context-and-commands")
    parser.add_argument("--unit", help="Process specific unit (e.g., 10-controlling-context)")
