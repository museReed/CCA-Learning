#!/usr/bin/env python3
"""
Generate visual study guides from video + SRT for Chapter 02-getting-started.
...
Processes both videos in 02-getting-started:
  - 07-adding-context (68 entries)
  - 08-making-changes (80 entries)
"""
...
VIDEO_CONFIG = [
    (
        "07-adding-context",
        "Adding_context_wWHe1PIF.srt",
        "Adding_context_wWHe1PIF.mp4",
        ADDING_CONTEXT_ZH,
        68,
        "Adding Context — 加入 Context",
    ),
    (
        "08-making-changes",
        "Making_changes_v2XzmjpP.srt",
        "Making_changes_v2XzmjpP.mp4",
        MAKING_CHANGES_ZH,
        80,
        "Making Changes — 進行更改",
    ),
...
def generate_markdown(entries: list[dict], unit_dir: str, title: str, output_path: Path):
    ...
    lines = [
        ...
        "| 項目 | 內容 |",
        "|------|------|",
        f"| 課程 | claude-code-in-action / 02-getting-started / {unit_dir} |",
        f"| 影片 | {title.split(' — ')[0]} |",
        f"| 字幕數 | {len(entries)} 段 |",
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate visual guides for 02-getting-started")
    parser.add_argument("--unit", help="Process specific unit (e.g., 07-adding-context)")
